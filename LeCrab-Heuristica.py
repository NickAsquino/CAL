...
class Enemy(Element):
	...
	def nextNode(self, target=None):
		if target is None:
			target = (len(MAP) - 2, len(MAP[0]) - 2)

		x, y = self.lastNode
		candidatos = []
		
		direcoes = [(x, y - 1), (x, y + 1), (x - 1, y), (x + 1, y)]

		for (nx, ny) in direcoes:
			if self.isSafeNode(nx, ny):
				h = abs(nx - target[0]) + abs(ny - target[1])
				candidatos.append(((nx, ny), h))

		if not candidatos:
			return None

		candidatos.sort(key=lambda c: c[1])
		melhor_no = candidatos[0][0]
	...
	def update(self):
		...
		if(mx == 0 and my == 0):
			px, py = P1.lastNode  
			self.destNode = self.nextNode(target=(px, py))
