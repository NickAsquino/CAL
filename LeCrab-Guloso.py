...
class Enemy(Element):
	...
	def nextNode(self, dirs=["UP", "DOWN", "LEFT", "RIGHT"]):
		...
		candidatos = [(d,coord) for d,coord in moves if self.isSafeNode(coord[0], coord[1])]

		if not candidatos:
			return None

		def manhattan(c):
			return abs(c[0] - px) + abs(c[1] - py)

		best_dist = min(manhattan(coord) for _, coord in candidatos)

		best_candidates = [(d,coord) for d,coord in candidatos if manhattan(coord) == best_dist]

		chosen_dir, chosen_coord = None, None
		for d,c in best_candidates:
			if d == self.direction:
				chosen_dir, chosen_coord = d, c
				break

		if chosen_coord is None:
			prev = getattr(self, "prevNode", None)
			non_back = [(d,c) for d,c in best_candidates if c != prev]
			if non_back:
				chosen_dir, chosen_coord = random.choice(non_back)
			else:
				chosen_dir, chosen_coord = random.choice(best_candidates)

		self.prevNode = self.lastNode
